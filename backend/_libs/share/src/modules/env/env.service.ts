...
const NODE_APP_TYPE = toEnum(
  E.NODE_APP_TYPE,
  Object.values(ClusterAppType),
  ClusterAppType.Web,
);
...
export class EnvService {
  ...
  SECRET_PASSWORD_SALT = toString(
    E.SECRET_PASSWORD_SALT,
  ...
  FRONT_MAIN_PROTOCOL = toEnum(
    E.FRONT_MAIN_PROTOCOL,
    Object.values(Protocol),
    Protocol.http,
  ) as Protocol;
  FRONT_MAIN_HOST = toString(E.FRONT_MAIN_HOST, 'example.com');
  FRONT_HIGHWINDS_HOST = toString(
    E.FRONT_HIGHWINDS_HOST,
    'highwinds-dev.jesusstream.com',
  );
  ...
  AWS_S3_BUCKET_NAME_CONTENT = toString(
    E.AWS_S3_BUCKET_NAME_CONTENT,
    'content',
  );

  IPFS_CACHE_DIR = toPath(E.IPFS_HOT_CACHE_DIR, './data/ipfs_cache');
...
export function toArrayStrings(
  envParam: string,
  spliter: string,
  defaultValue: string[],
) {
